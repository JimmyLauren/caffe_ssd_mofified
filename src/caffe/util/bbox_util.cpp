...
void GetDetectionResults(
	const Dtype* det_data, const int num_det,
...
void GetDetectionResults(
	const Dtype* det_data, const int num_det,
	const int background_label_id,
	map<int, map<int, vector<NormalizedBBox> > >* all_detections, vector<pair<int, int>>& old2new, bool isMap) {
	all_detections->clear();
	for (int i = 0; i < num_det; ++i) {
		int start_idx = i * 7;
		int item_id = det_data[start_idx];
		...
		if (isMap) {  
			...
			tempLabel=(int)it->second;//得到旧的it->first对应的新的it->second标签
		}
		else
			tempLabel=label;
		label = tempLabel;
