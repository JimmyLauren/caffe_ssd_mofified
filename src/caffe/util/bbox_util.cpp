...
// need input of tp(true positives) and fp(false positives)
void ComputeAP(const vector<pair<float, int> >& tp, const int num_pos,
               const vector<pair<float, int> >& fp, const string ap_version,
               vector<float>* prec, vector<float>* rec, float* ap)//, vector<float> *temp)//增加一个容器变量temp来存储11个精度值
 {
  //prec is for precision; rec is for recall; ap is for average precision.
