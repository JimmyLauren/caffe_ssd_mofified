...
void Solver<Dtype>::TestDetection(const int test_net_id) {
	...
	std::ofstream OutFile(file.c_str(), ios::app);//app means append
	  ...
	  CHECK_NOTNULL(test_nets_[test_net_id].get())->
		  ShareTrainedLayersWith(net_.get());
	  // \B5\DAһ\B8\F6int\B1\EDʾimage id\A3\AC\B5ڶ\FE\B8\F6int\B1\EDʾlabel
	  ...
	  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
		...
		for (int j = 0; j < result.size(); ++j) {//\B6\D4\D3\DA\CB\F9\D3е\C4\CA\E4\B3\F6blob


 

			  CHECK_EQ(result[j]->width(), 5); //in detection evaluate layer, the outcomes are saved in 5
			  const Dtype* result_vec = result[j]->cpu_data();//\B4\FA\B1\ED\B6\D4һ\D5\C5ͼ\CF\F1\B5\C4Ԥ\B2\E2
			  int num_det = result[j]->height(); //nchw\D6У\ACwĽ\E1\B9\FBht\B1\EDʾ\B9\B2\D3ж\E0\C9ٸ\F6Ԥ\B2\E2\B5Ľ\E1\B9\FB\A1\A3
			  for (int k = 0; k < num_det; ++k) {
					int item_id = static_cast<int>(result_vec[k * 5]); //ͼƬ\B5\C4ID
	  ...
	  //\B1\A3\B4\E6\D5\FD\BC\EC\CA\FDtp,\B4\ED\BC\EC\CA\FDfp\A3\AC©\BC\EC\CA\FDfn
	  ...
	  for (int i = 0; i < all_true_pos.size(); ++i) {
			...
			for (map<int, int>::const_iterator it = num_pos.begin();it != num_pos.end(); ++it) 
			{
				  ...
				  // \D4\DA\E3\D0ֵ\CF¼\C6\CB\E3recall\BA\CDprecision\A3\AC\B2\A2\B4\F2ӡ\B3\F6\C0\B4
				  float thr = param_.rec_prec_thr(); // \BF\C9\D4\DAsolver\D6ж\A8\D2\E5\B5\C4\E3\D0ֵ\B2\CE\CA\FD
			...
			// \B4\F2ӡ\B3\F6\CB\F9\D3\D0\C0\E0\B1\F0\D4\DAһ\B6\A8\E3\D0ֵ\CFµ\C4recall
			LOG(INFO) << "-------------recalls-----------";
			...
			// \B4\F2ӡ\B3\F6\CB\F9\D3\D0\C0\E0\B1\F0\D4\DAһ\B6\A8\E3\D0ֵ\CFµ\C4precision
			LOG(INFO) << "-------------precisions-----------";
	...
	//\BD\ABÿһ\C0\E0\B5\C4\D5\FD\BC\EC\CA\FDtp,\B4\ED\BC\EC\CA\FDfp\A3\AC©\BC\EC\CA\FDfn\B1\A3\B4浽\CEļ\FE
	OutFile << "---------------------------" << std::endl;
